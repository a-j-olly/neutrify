...
export class AppComponent {
	@ViewChild('filterMenuContainer', { read: ViewContainerRef })
	...
	public async detectTheme(): Promise<void> {
		return await this.themeDetection
			.isAvailable()
			.then((res: ThemeDetectionResponse) => {
				if (res.value) {
					this.themeDetection
						.isDarkModeEnabled()
						.then((theme: ThemeDetectionResponse) => {
	...
	private loadFilterMenu() {
		...
		try {
			...
			import('./menu/filter-menu/filter-menu.module').then(
			// eslint-disable-next-line @typescript-eslint/naming-convention
				({ FilterMenuModule }) => {
					// Compile the module
					this.compiler
						.compileModuleAsync(FilterMenuModule)
						.then((moduleFactory) => {
							// Create a moduleRef, resolve an entry component, create the component
							const moduleRef = moduleFactory.create(this.injector);
							const filterMenuFactory =
								moduleRef.instance.resolveFilterMenuComponent();
							// eslint-disable-next-line @typescript-eslint/consistent-type-assertions
							<ComponentRef<FilterMenuComponent>>(
								this.filterMenuContainer.createComponent(
									filterMenuFactory,
									null,
									moduleRef.injector
								)
      ...
      this.hasFilterMenuViewInit = false;
      console.warn('Filter menu failed to init', e.message);
    }
	}
}
