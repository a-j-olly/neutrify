export class DelayedScriptLoader {
	...
	constructor( urls: string[] | string, delayInMilliseconds: number );
		...
		this.scriptPromise = this.delay( this.delayInMilliseconds )
			.then(
				() => {
					...
					const scriptPromises = this.urls.map(
						( url ) => ( this.loadScript( url ) )
					);
	...
	private loadScript( url: string ): Promise<any> {
		...
		const promise = new Promise(( resolve, reject ) => {
			...
			const commentNode = document.createComment( 'Script injected via DelayedScriptLoader.' );
			...
			const gtagNode = document.createElement( 'script' );
			gtagNode.innerHTML = ( window as any ).dataLayer = ( window as any ).dataLayer || [];


			// eslint-disable-next-line prefer-arrow/prefer-arrow-functions
			const func: (key: string, date: Date) => unknown = function gtag() {(window as any).dataLayer.push(arguments);};
