...
export class NewsFeedService {
	...
	private limit = 25;
	private searchFilterData$ = new Subject<{
		searchTerm: string;
		useFilters?: boolean;
	}>();
	...
	public async listArticles(
		...
		try {
			...
			console.log(
				'Could not get articles. Service returned this error: ',
				error
			);
			...
			if (error.errors && error.errors.length === 1) {
				...
				if (errMes.message === 'Network Error') {
					for (let i = 0; i < 3; i++) {
						...
						if (i === 2) {
							alert(
								"Couldn't recover from network difficulties. Please check your connection."
							);
						}
					}
				}
	...
	public async handleInitDataLoad() {
		...
		do {
			if (i === 1) {
				...
				newLimit = 1000;
				this.readyArticles.push(
					...(await this.listArticles(newLimit, this.nextToken))
				);
			} else {
				await this.presentToast(
					'There are not that many articles that fit your criteria.',
					'primary'
				);
				break;
			}
			...
			this.nextToken &&
			this.readyArticles.length < this.displayThreshold
		);

		this.displayArticles = this.readyArticles.slice(
			0,
			this.displayThreshold - 1
		);
		...
		if (!this.nextToken && this.readyArticles.length < this.displayThreshold) {
			await this.presentToast(
				'Could only find a few articles that fit your criteria. Try to remove some filters.',
				'primary'
			);
		}
	}
	...
	public async getNextPage() {
		if (this.nextToken && this.readyArticles.length < this.displayThreshold) {
			...
			this.presentToast(
				"There are no more articles to be read. You're up to date.",
				'primary'
			);
		}
	...
	public async saveFilters() {
		...
		const res = await this.filterService.saveFilters(
			this.authService.signedIn ? false : true
		);
	...
	public async loadFilters() {
		this.filterService.updateFilterLoading(true);
		const res = await this.filterService.loadFilters(
		...
		if (res) {
			await this.presentToast(
				'Your changes to the filters have been reset.',
				'success'
			);
			this.ga.eventEmitter('load_filters', 'engagement', 'Re-loaded filters');
		} else {
			this.presentToast(
				'Could not reset your filters. Please try again.',
				'danger'
			);
		}
	}
	 ...
	 */
	public setSearchFilter(searchFilterData: {
		searchTerm: string;
		useFilters?: boolean;
	}) {
	...
	private async earlyImageLoad(articleArray: Array<Article>) {
		const n =
			this.readyArticles.length * 0.33 > 21
				? 21
				: this.readyArticles.length * 0.33;
		const loadingImages: Promise<void>[] = articleArray
			.slice(0, n)
			.map(async (article) => {
				if (article.image) {
					try {
	...
	private async loadReadyArticles() {
		...
		do {
			...
			newArticles.push(
				...(await this.listArticles(this.limit, this.nextToken))
			);
	...
	private async loadDisplayArticles() {
		...
		if (this.readyArticles.length >= this.displayThreshold) {
			...
			this.displayArticles.push(
				...this.readyArticles.slice(0, this.displayThreshold - 1)
			);
			this.readyArticles = this.readyArticles.slice(this.displayThreshold - 1);
		...
		if (this.displayArticles.length >= 3 * this.displayThreshold) {
			...
			if (
				this.openArticleIndex &&
				this.openArticleIndex - (numNewArticles - 1) < 0
			) {
	 ...
	 */
	private setSearchFilterData(data: {
		searchTerm: string;
		useFilters?: boolean;
	}) {
	 ...
	 * @param  {boolean} menuOpen - Is the menu open?
	 */
	private calculateThresholdMultiplier(
		platformWidth: number,
		menuOpen: boolean
	) {
